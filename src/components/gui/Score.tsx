...
import { CircularProgressLabel, Center } from "@chakra-ui/react";
...
function Counter({ totals, percent, label, color, text }: { totals: number, percent: number, label: number, color: string, text: string }) {
    ...
    return (
        <Box p={5} borderWidth="2px" alignContent="center" borderRadius="lg" w={200} mb={4}>
            <VStack>
                <CircularProgress size="120px" value={percent} color={c}>
                    <CircularProgressLabel color={c} className="score" fontSize="5xl" fontWeight="bold">{label}</CircularProgressLabel>
                </CircularProgress>
